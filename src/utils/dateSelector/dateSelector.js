...
(function (wid, dcm) {
  ...
  DateSelector.prototype = {
    ...
    checkTime () {
      ...
      let recent_time = this.recent_time;
      ...
      if (_this.idxArr.length === _this.beginTime.length && _this.beginTime.length === _this.endTime.length && _this.endTime.length === _this.recentTime.length) {
        ...
        let bt = new Date(begin_time[0], begin_time[1], begin_time[2], begin_time[3], begin_time[4]).getTime();
        let et = new Date(end_time[0], end_time[1], end_time[2], end_time[3], end_time[4]).getTime();
        let rt = new Date(recent_time[0], recent_time[1], recent_time[2], recent_time[3], recent_time[4]).getTime();
        if (rt < bt) {
          win.alert('当前时间小于开始时间')
        }
        if (rt > et) {
          win.alert('当前时间超过结束时间')
        }
        return (bt <= rt && rt <= et);
      } else {
        win.alert('error,please open the console to see the errmsg');
    ...
    initReady () {
      ...
      loop(0, _this.ulCount, function (i) {
        ...
        let min = 0;
        let max = 0;
        let tempDomUl = $id('date-selector-' + _this.container + '-' + _this.idxArr[i]);
        let tempArray = _this['array' + _this.idxArr[i]] = [];
        switch (realIdx) {
          case 0:
    ...
    initBinding () {
      let _this = this;
      let bg = $id('date-selector-bg-' + _this.container);
      let container = $id('date-selector-container-' + _this.container);
      let body = doc.body;
      ...
      on('touchstart', 'date-selector-tab', function (event) {
        let tab = $class('date-selector-tab');
        let content = $class('date-selector-content');
    ...
    initCommonArr (tempDomUl, tempArr, min, max, str, idx) {
      let _this = this;
      let Html = '';
    ...
    initRangeArr (min, max, str, checkIdx, dir) {
      let _this = this;
      let realIdx = _this.idxArr[checkIdx];
      let arr = [];
      let $selector = $id('date-selector-' + _this.container + '-' + realIdx);
      let targetLong = 0;
    ...
    checkRange (checkIdx) {
      let _this = this;
      if (checkIdx >= _this.ulCount - 1) {
        return;
      }
      let min = 0;
      let max = 0;
      let str = '';
      let dir = 0; //0在顶部,1在底部,-1在中间
      let realIdx = _this.idxArr[checkIdx];
      switch (realIdx) {
        case 0:
          min = 1;
      ...
      loop(0, checkIdx + 1, function (p) {
        if (_this.beginTime[p] !== _this.resultArr[p]) {
          ...
          loop(0, checkIdx + 1, function (q) {
            if (_this.endTime[q] !== _this.resultArr[q]) {
              dir = -1;
            }
          });
        }
    ...
    initSpeed (arr, dir, max, idx) {
      ...
      let rate = 0;
      if ((variance / arr.length).toFixed(2) > 0.1) {
    ...
    touch (event, that, $selector, array, idx) {
      ...
      switch (event.type) {
        ...
        case "touchend":
          ...
          let tempRes = that.end.index = that.distance[idx] / that.liHeight + 2;
